...
One peculiar thing I noticed from this exercise is the steady states for the adjustment costs are 0, even though there is positive price and wage inflation at steady state. The values are low enough that it makes some sense it wouldn't make a large quantitative difference, but other variables are solved with much greater precision. This is consistent with the steady state values that are coded in the original replication files. Trying different values for the asymmetry parameters revealed that they made no observable difference in the impulse response functions, even under a second order approximation. If you enter the model equations in levels, this does not occurr with or without the log-linear option, which is a comprable approach to coding everything in log levels. 

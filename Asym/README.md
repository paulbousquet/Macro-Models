...
The first baseline is simply keeping with the syntax of the original files (carrying around the exponenetial terms as descrobed above). One peculiar thing I noticed from this exercise is that trying different values for the asymmetry parameters revealed that they made no observable difference in the impulse response functions, even under a second order approximation. If you enter the model equations in levels, this does not occurr with or without the log-linear option, which is a comprable approach to coding everything in log levels.  IRFs using the equations as written from ABS do produce some non-linearity -- the impulse responses to a 2 standard deviation shock are not simply the paths from the 1 standard deviation response multiplier by 2 -- but otherwise, not much non-linear action occurrs. When entering the model in levels, you get much more of the spirit of the rigity structure: when you reduce the price asymmetry parameter (and keep it fixed for wages), prices adjust comparatively more following a monetary policy shock that puts downward pressure on prices and wages. However, at a first order approximation, the impulse responses to a positive and negative monetary policy shock, which put opposing directional pressure on prices, are still somehow mirror images. It's only at a second order approximation of the IRFs that the asymmetry can be properly observed. 

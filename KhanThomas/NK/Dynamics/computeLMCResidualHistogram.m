...
global mStateGrid ttheta nnu ddelta mStatePoly vStatePolySquared tolerance maxIterations nState mFinePoly mFineGrid ...
	...
	nProdFine nCapitalFine vShocksWeights ppsiCapital nSS intermediateGoodsPriceSS
...
vLaborDemandGrid 	= ((intermediateGoodsPriceSS * exp(mStateGrid(:,1)) .* (mStateGrid(:,2) .^ ttheta) * nnu) / wage) .^ (1 / (1 - nnu));
vProfitGrid 				= intermediateGoodsPriceSS * exp(mStateGrid(:,1)) .* (mStateGrid(:,2) .^ ttheta) .* (vLaborDemandGrid .^ nnu) - wage * vLaborDemandGrid;

% Initialize value function (warm-start from previous call if available)
persistent vCoefficientsWarmStart_Histogram
if isempty(vCoefficientsWarmStart_Histogram)
	...
	vCoefficients 			= vCoefficientsWarmStart_Histogram;
end
...
vCoefficientsWarmStart_Histogram = vCoefficients;
...
while err > 1e-10 && iterations < 1e4
...
vLaborDemand 			= ((intermediateGoodsPriceSS * exp(mFineGrid(:,1)) .* (mFineGrid(:,2) .^ ttheta) * nnu) / wage) .^ (1 / (1 - nnu)) + ((vCutoff .^ 2) ./ (2 * ppsiCapital));

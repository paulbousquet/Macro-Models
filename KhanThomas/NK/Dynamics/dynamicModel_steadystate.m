...
% NK steady state values
intermediateGoodsPrice = intermediateGoodsPriceSS;	% (gamma-1)/gamma
inflation = 0;
nominalInterestRate = nominalInterestRateSS;			% 1/beta - 1
expectedInflation = 0;
tobinQ = 1;

% NK aggregate level variables -- use values from computeLMCResidualPolynomials
% for internal consistency (these must match the policies that determine the
% promoted auxiliary variables later in this file)
aggregateOutputLevel = aggregateOutput;
aggregateInvestmentLevel = aggregateInvestment;
aggregateCapitalStock = aggregateCapital;

% Investment resource cost (at SS, tobinQ=1, so cost = I)
investmentResourceCost = aggregateInvestmentLevel;

% Recompute consumption from NK resource constraint for consistency
% At SS: C = Y*(1-varphi/2*pi^2) - investmentResourceCost = Y - I
aggregateConsumption = aggregateOutputLevel - investmentResourceCost;
...
% Log NK variables
logInflation = 0;								% log(1+0) = 0
logNominalInterestRate = log(1 + nominalInterestRateSS);
logTobinQ = 0;									% log(1) = 0
logIntermediateGoodsPrice = log(intermediateGoodsPriceSS);
...
% At steady state: tobinQ = 1, so marginalUtility * tobinQ = marginalUtility
...
% Find cchi index by name (robust to parameter reordering)
cchiIdx = find(strcmp(M_.param_names, 'cchi'));
for iParameter = cchiIdx:length(M_.params)
    paramname = M_.param_names{iParameter,:};
